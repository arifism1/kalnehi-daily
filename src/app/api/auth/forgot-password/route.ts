...
function authCallbackUrl(request: NextRequest, nextPath: string): string {
...
export async function POST(request: NextRequest) {
  ...
  if (check.error && check.error !== "invalid email" && check.error !== "invalid ip") {
    return NextResponse.json(
      { error: "Could not verify reset limits. Try again shortly.", code: "rate_limit_error" },
      { status: 503 },
    );
  }
  ...
  const { error: resetErr } = await supabase.auth.resetPasswordForEmail(email, {
    redirectTo: authCallbackUrl(request, "/auth/reset"),
  });
  ...
  return NextResponse.json(
    { error: formatSupabaseError(resetErr), code: "auth_error" },
    { status: 400 },
  );
}
