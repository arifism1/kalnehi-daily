...
/** Keeps uniform scaling — avoid combining aspect-ratio + separate max-height on the same box with fill-stretched SVGs. */
const FRAME =
  "flex w-full justify-center rounded-2xl border border-white/[0.07] bg-[#0D0905]/35 " +
  "p-2 shadow-[0_16px_56px_rgba(0,0,0,0.45)] ring-1 ring-white/[0.04] sm:rounded-3xl sm:p-3";

/**
 * Night sky with staggered stars + crescent (Framer Motion). Inline SVG, no images.
 * Intrinsic 390×220 size with max-height + width:auto scaling preserves circles (no horizontal shear).
 */
export function NightIllustration({ className }: { className?: string }) {
  return (
    <div className={className} aria-hidden>
      <div className={`mx-auto w-full max-w-4xl lg:max-w-5xl ${FRAME}`}>
        <svg
          width={390}
          height={220}
          viewBox="0 0 390 220"
          ...
          className="mx-auto block h-auto w-auto max-w-full md:max-h-[min(240px,28vh)] lg:max-h-[min(260px,26vh)] xl:max-h-[min(280px,28vh)]"
        >
          <defs>
