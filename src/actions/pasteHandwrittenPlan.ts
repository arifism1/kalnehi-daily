...
export async function parsePastedHandwrittenPlan(
  ...
  const raw = pastedText.trim().slice(0, 30_000);
  if (!raw) {
    return { ok: false, error: "Paste some text first.", tasks: [] };
  }
  ...
  try {
    ...
    const completion = await groq.chat.completions.create({
      ...
      messages: [
        ...
        {
          role: "user",
          content: `Extract rows from this pasted text:\n\n${raw}\n\nReturn only the JSON array.`,
        },
      ],
    });
    ...
    console.log("Groq pasted-plan parse error:", e instanceof Error ? e.message : String(e));
  }
